    ...
    public String calculateChange(
        ...
        switch (calcType) {
            ...
            case "difference" -> {
                ...
                return String.format("차이: %s - %s = %s (%s)",
                        formatNumber(compareValue), formatNumber(baseValue),
                        formatNumber(diff), direction);
            }
            case "ratio" -> {
                if (baseValue == 0) {
                    return "계산 불가: 기준값이 0이므로 비율을 계산할 수 없습니다.";
                }
                double ratio = compareValue / baseValue;
                return String.format("비율: %s / %s = %.1f배",
            ...
            case "percentage_point_change" -> {
                ...
                return String.format("변동폭: %s - %s = %.1f%%p (%s)",
                        formatNumber(compareValue), formatNumber(baseValue),
                        change, direction);
            }
            default -> {
